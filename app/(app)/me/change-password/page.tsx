...
async function changePasswordAction(formData: FormData) {
  ...
  let meId = (session.user as { id?: string }).id as string | undefined
...
export default async function ChangePasswordPage() {
  ...
  return (
    <section className="space-y-4">
      ...
      <form action={changePasswordAction} className="max-w-md space-y-3 rounded border border-divider p-4 bg-content1">
        <div className="flex flex-col gap-1">
          <label className="text-sm">Текущий пароль</label>
          <input name="currentPassword" type="password" required className="border border-divider rounded px-3 py-2 bg-background text-foreground" />
        </div>
        <div className="flex flex-col gap-1">
          <label className="text-sm">Новый пароль</label>
          <input name="newPassword" type="password" minLength={6} required className="border border-divider rounded px-3 py-2 bg-background text-foreground" />
        </div>
        <div className="flex flex-col gap-1">
          <label className="text-sm">Подтверждение</label>
          <input name="confirm" type="password" required className="border border-divider rounded px-3 py-2 bg-background text-foreground" />
        </div>
        <button className="inline-flex items-center rounded border border-divider px-3 py-1 text-sm hover:bg-content2 bg-content1 text-foreground">Сменить пароль</button>
      </form>
    </section>
  )
}
