...
export async function createPointAction(fd: FormData): Promise<Result> {
  ...
  if (!session?.user || (session.user as { role?: string }).role !== 'PARTNER') {
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      const exists = await tx.user.findFirst({
        where: { OR: [ { login }, email ? { email } : undefined ].filter(Boolean) as { login?: string; email?: string }[] },
      ...
      const safeEmail = email ?? `${login}@local.generated`;
    ...
    try {
      ...
      cookieStore.set('point.oneTime', JSON.stringify({ pointId: result.pointId, password }), { httpOnly: true, sameSite: 'lax', path: '/partner/points', maxAge: 60 });
    } catch {}
    return { ok: true };
  } catch (e: unknown) {
    const error = e as { code?: string; meta?: { target?: string }; message?: string };
    console.error('[createPointAction] error:', error?.code, error?.meta, error?.message)
    if (error?.code === 'P2002') {
      const t = error?.meta?.target || 'поле';
      return { ok: false, message: `Значение ${t} уже занято` };
    }
    if (error?.code === 'P2003') return { ok: false, message: 'Нарушение внешнего ключа (партнёр или аккаунт не найден)' };
    if (error?.code === 'P2014') return { ok: false, message: 'Неверные связи (relation violation)' };
    if (error?.code === 'P2025') return { ok: false, message: 'Запись не найдена при создании связи' };
    return { ok: false, message: error?.message || 'Не удалось создать точку' };
  }
}
