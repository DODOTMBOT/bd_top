...
function translitCyrToLat(s: string) {
  ...
  return s.split('').map((ch) => {
    ...
    const t = (map as Record<string, string>)[low];
...
function randomDigits(n: number) {
  ...
  let s = '';
  for (let i = 0; i < n; i++) s += ((bytes[i] ?? 0) % 10).toString();
  return s;
}
...
export async function createEmployeeAction(formData: FormData): Promise<void> {
  ...
  if (!fullName) throw new Error('Укажите имя сотрудника')
  ...
  const payload: string = `${created.id}:${created.email}:${password}`
  const cookieStore = await cookies()
  cookieStore.set('flash.empCreated', payload, {
    httpOnly: true,
    sameSite: 'lax',
...
export async function ackEmpNotice() {
  const cookieStore = await cookies()
  cookieStore.set({ name: 'emp_notice', value: '', maxAge: 0, path: '/' })
...
export async function consumeEmpFlash() {
  'use server'
  const cookieStore = await cookies()
  cookieStore.delete('flash.empCreated')
}
