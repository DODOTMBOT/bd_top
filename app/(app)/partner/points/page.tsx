...
export default async function PartnerPointsPage({ searchParams }: { searchParams?: Promise<Record<string, string | undefined>> }) {
  try {
    await requirePartnerFromSession()
    ...
    if (error?.code === 401 || error?.message === "PARTNER_NOT_FOUND_FOR_ACCOUNT" || error?.message === "Partner not found") {
      return (
